...
## 
# @file rua_IMU_adapter.py 
# @author Davide Piccinini <piccio98dp\@gmail.com>
# @date 24 March 2021
# @brief This file contains the right upper arm IMU adapter.
...
if __name__ == "__main__":
    try:
        rospy.init_node('rua_IMU_adapter')

        script_path = os.path.abspath(__file__) 
        path_list = script_path.split(os.sep)
        script_directory = path_list[0:len(path_list)-2]
        csv_path = rospy.get_param("rua_csv_path")
        path = "/".join(script_directory) + "/" + csv_path
        csvFile = open(path, 'r')
        imuData = csv.reader(csvFile)

        samplePub = rospy.Publisher("rua_IMU_data", ImuSample, queue_size=1)
